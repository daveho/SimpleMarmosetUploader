...
public abstract class Uploader {
	...
	static {
		Protocol easyHttps = new Protocol("myhttps", (ProtocolSocketFactory) new EasySSLProtocolSocketFactory(), 443);
		Protocol.registerProtocol("myhttps", easyHttps);

		Protocol easyHttps2 = new Protocol("myhttps2", (ProtocolSocketFactory) new EasySSLProtocolSocketFactory(), 8443);
		Protocol.registerProtocol("myhttps2", easyHttps2);
	}
	...
	public static Result sendZipFileToServer(Properties submitProperties, File zipFile, String username, String password) throws HttpException, IOException {
		...
		// HACK try to allow use of self-signed (or otherwise dodgy)
		// SLL certificates.  Seems to work ok.
		if (proto.equals("https:")) {
			if (portNumber.equals("")) {
				proto = "myhttps:";
			} else {
				// XXX: only works if server is listening on port 8443
				proto = "myhttps2:";
			}
		}
